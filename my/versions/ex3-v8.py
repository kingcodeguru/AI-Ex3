# thompson version with 5 more optimization ideas + better hyperparameters + enable exit not to goal floor
"""AI assistance disclosure: drafted with Gemini.

The Ultimate Bayesian RL Controller (High-Compute / High-Reward Edition).
Fixed the "Transfer Trap" bug so A* correctly delegates multi-elevator handoffs.
Maxed out computational hyperparameters to trade spare CPU time for optimal rewards.
"""
...
# ==============================================================================
# ⚙️ HYPERPARAMETERS & CONFIGURATION (HIGH-COMPUTE EDITION)
# ==============================================================================
ENSEMBLE_SIZE = 15              # Tripled. Massively improves stability of Bayesian votes.
ASTAR_NODE_LIMIT = 8000         # Quadrupled. Allows deep lookahead for complex transfers (like e3).
GAMMA = 0.95                    # Highly patient. Agent will pursue distant, lucrative targets.
PRIOR_BETA_FAILURES = 0.1       # Extreme initial optimism.
INITIAL_ASSUMED_REWARD = 60.0   # High initial reward assumption encourages full exploration.
ENDGAME_STEPS_THRESHOLD = 50    # Stops exploring slightly earlier to lock in points.
SUBSET_FARMING_ROI_BOOST = 1.08 # Eagerly enters infinite loops if an 8% ROI advantage is found.
MIN_PROBABILITY_CLAMP = 0.005   # Clamp for numerical stability.
MIN_REWARD_VARIANCE = 0.05      # Clamp for Gaussian sampling.
# ==============================================================================
...
class Controller:
    def __init__(self, game: ext_elev.GameAPI):
        ...
        # Identify shared floors for multi-elevator transfers (Fixes the e3 Bug)
    ...
    def choose_next_action(self, state):
        ...
        # Draw the TRUE MEAN expected model to evaluate subsets statically
        ...
        for _ in range(ENSEMBLE_SIZE):
            ...
            ps = (state[0], state[1])
            plan = self._astar_plan(ps, farming_targets, s_ep, s_pp)
    ...
    def _astar_heuristic(self, ps, targets):
        elevs, persons = ps
    ...
    # ------------------------------------------------------------------
    # Deep Lookahead (Bounded A* Planner)
    # ------------------------------------------------------------------
    def _plan_successors(self, ps, targets, p_prob, e_prob):
        ...
        # Fix: Allow A* to drop people off at shared transfer floors!
        for pid, loc in persons:
            if pid not in targets or loc[0] != "in": continue
            eid = loc[1]
            ef = e_dict[eid][0]
            if ef != self.goals[pid] and ef in self._shared_floors:
                ...
                succ.append((f"EXIT{{{pid},{eid}}}", (n_elevs, n_pers), cost))

        # Fix: Ensure A* considers shared floors as interesting destinations!
    ...
    def _astar_plan(self, ps, targets, e_prob, p_prob):
        h0 = self._astar_heuristic(ps, targets)
