# gemini version + supposably fast
"""AI assistance disclosure: drafted with Gemini.

Blazingly Fast Hybrid RL controller for the stochastic multi-elevator MDP.
Optimized state transitions (no sorting) and internal tuple-based action 
representations to maximize Expectimax and A* node expansion rates.
"""
...
class Controller:
    ...
    def choose_next_action(self, state):
        ...
        if current_step < self._explore_steps:
            ...
            self._clear_caches()
            action_tuple = self._expectimax_action(state, steps_left, forced_depth=2)
    ...
    def _update_stats(self, state):
        if self._last_action is None or self._last_action[0] == "RESET":
            return
            
        is_reset = (state[2] == len(self._person_ids) and self._last_state[2] == 1)
        kind, p1, p2 = self._last_action
    ...
    def _update_ucb_estimates(self):
        for eid in self._elevator_ids:
            ...
            self._elevator_prob[eid] = (suc + 1.0) / (att + 1.0) 
        ...
        for pid in self._person_ids:
            ...
            if not rews:
                ...
                self._person_mean_reward[pid] = (sum(rews) / len(rews)) + (20.0 / math.sqrt(len(rews)))
    ...
    def _setup_exploitation_planner(self):
        ...
        _floors_all = set().union(*(self._reachable_sets[_e] for _e in self._elevator_ids))
        ...
        self._mode = "expectimax" if self._force_expectimax else self._select_mode()
    ...
    def _choose(self, state, steps_left, mode):
        ...
        plan = self._get_plan((state[0], state[1]))
    ...
    def _legal_actions(self, state):
        ...
        for eid in self._elevator_ids:
            ...
            candidates = [p for p in people_on_floor.get(floor, []) if load + self._person_weight[p] <= self._capacities[eid]]
        ...
        for eid in self._elevator_ids:
            ...
            for _, target in targets[:4 if len(targets) <= 4 else 3]:
    ...
    # ------------------------------------------------------------------
    # Transition model (Highly Optimized Tuple Logic)
    # ------------------------------------------------------------------
    ...
    def _heuristic(self, state, steps_left):
        ...
        for pid, loc in persons_t:
            ...
            total += self._person_mean_reward[pid] * (0.95 ** min(cost, steps_left))
            total_cost += cost
