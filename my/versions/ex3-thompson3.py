# thompson version with 5 more optimization ideas + hyperparameters above
"""AI assistance disclosure: drafted with Gemini.

The Ultimate Bayesian RL Controller (Fixed ROI Farming).
Integrates Ensemble Thompson Sampling, Deep Lookahead A* Planning,
Gaussian Reward Posteriors, Horizon-Aware Exploitation, and Strict Subset Farming.
"""
...
# ==============================================================================
ENSEMBLE_SIZE = 5               # Reduced slightly for blazing execution speed
ASTAR_NODE_LIMIT = 2000         # Prevents timeouts on deep combos
GAMMA = 0.90                    # Discount Factor
PRIOR_BETA_FAILURES = 0.1       # Optimistic initial exploration
INITIAL_ASSUMED_REWARD = 40.0   # Forces exploring untested people at least once
ENDGAME_STEPS_THRESHOLD = 45    # Steps left before crushing variance to 0 (Exploit)
SUBSET_FARMING_ROI_BOOST = 1.15 # 15% better ROI needed to abandon the map and farm
MIN_PROBABILITY_CLAMP = 0.01    # Prevents math explosion on bad elevators
MIN_REWARD_VARIANCE = 0.1       # Prevents divide-by-zero in Gaussian sampling
# ==============================================================================
...
class Controller:
    ...
    # ------------------------------------------------------------------
    # Static Combinatorial Subset Farming (Fixed Amnesia Bug)
    # ------------------------------------------------------------------
    def _evaluate_subsets(self, m_rew):
        """Evaluates ROIs exclusively from the initial state to find permanent loops."""
        ...
        # 1. Base ROI (Clear entire map, which grants goal_reward and auto-resets)
        ...
        max_sub = len(self.person_ids) - 1
        if max_sub < 1: return self.all_targets
        
        # 2. Subset ROIs (Farm specific people and manually RESET)
        for r in range(1, max_sub + 1):
            for subset in itertools.combinations(self.person_ids, r):
                ...
                if subset_cost < INF:
                    subset_cost += 1.0  # Exact 1 step penalty for executing RESET
    ...
    def _astar_heuristic(self, ps, targets):
        """Fixed heuristic: accurate expected costs without doubling."""
        elevs, persons = ps
